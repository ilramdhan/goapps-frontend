...
import { useImportMbLustures, useDownloadMbLustureTemplate } from "@/hooks/finance/use-mb-lusture"
import { readFileAsBytes } from "@/lib/api"

interface MbLustureImportDialogProps {
  open: boolean
...
export function MbLustureImportDialog({
  open,
  onOpenChange,
  onSuccess,
}: MbLustureImportDialogProps) {
  const fileInputRef = useRef<HTMLInputElement>(null)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [duplicateAction, setDuplicateAction] = useState<DuplicateAction>("skip")
  const [importResult, setImportResult] = useState<ImportResult | null>(null)

  const importMutation = useImportMbLustures()
  const templateMutation = useDownloadMbLustureTemplate()
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[550px]">
        <DialogHeader>
          <DialogTitle>Import MB Lustures</DialogTitle>
          <DialogDescription>
            Import MB Lustures from an Excel file. Download the template first to
